...
tags_next(char *tags, char **tag, char **val)
{
	...
	CHECK(tags != NULL, "tags_next() given a NULL tag string to iterate over");
	CHECK(tag != NULL,  "tags_next() given a NULL destination pointer for the next tag");
	CHECK(val != NULL,  "tags_next() given a NULL destination pointer for the next value");

	CHECK(s_iskeystart(*tags), "tags_next() given a malformed tag string");
